...
def sample_shape_space(net, key, chd_type, cfg, tester, lat_vecs=None, stats=None, num_copies=10, num_block=1, thresh=0.5, get_img=False, get_noCorr=False):
    import random
    filename = key
    
    if net.use_diag or (not hasattr(net, 'type_encoder')):
        # If use_diag is true OR type_encoder is missing,
        # use chd_type directly.
        # This covers the case where type_encoder was not created due to num_types=1.
       #z_t = chd_type
        z_t = torch.from_numpy(np.array([chd_type]).astype(np.float32)).to(device)
    else:
        # Use type_encoder only if use_diag is false AND type_encoder exists.
        z_t = net.type_encoder(torch.from_numpy(np.array([chd_type]).astype(np.float32)).to(device))
    print('len: ',len(cfg['data']['chd_info']['types']))
    type_sdf = tester.z2voxel(None, None, z_t, net, num_blocks=num_block, out_type=True)
    print('type_sdf shape: ', type_sdf.shape)
    import sys
    sys.exit()
...
def save_type_net(net, path):
    checkpoint = {}
    if not net.use_diag and hasattr(net, 'type_encoder'):
...
def image_mesh_synthesize(net, lat_vecs, stats, num_copies=1,num_block=1, thresh=0.5, cfg=None, tester=None):
    ...
    train = dataset.ImgSDFDataset(cfg['data']['train_dir'], cfg['train']['n_smpl_pts'], chd_info=cfg['data'].get('chd_info', None), mode=['train'], use_aug=False)
...
def get_vsd_variations(cfg, mode, net, code, vsd_id, num_var=10, thresh=0.5):
    train = dataset.SDFDataset(cfg['data']['train_dir'], cfg['train']['n_smpl_pts'], cfg['data']['output_dir'], chd_info=cfg['data'].get('chd_info', None), mode=MODE, use_aug=False, pad_num=0)
...
if __name__ == '__main__':
    ...
    # TRAINING ACCURACY
    train = dataset.SDFDataset(cfg['data']['train_dir'], cfg['train']['n_smpl_pts'], cfg['data']['output_dir'], chd_info=cfg['data'].get('chd_info', None), mode=MODE, use_aug=False, pad_num=cfg['train']['pad_num'])
    ...
    if test_ops['type_pred'] and cfg['data'].get('chd_info'):
        # TEST 1: TYPE PREDICTION
        test_type_prediction(net, cfg['data']['chd_info'], thresh=THRESH)

    if test_ops['type_interp'] and cfg['data'].get('chd_info'):
    ...
    if test_ops['train_dice']:
        ...
        with torch.no_grad():
            for i, data in enumerate(dataloader_test):
                original_copy = bool(re.match("pulse_\d+_artery", data['filename'][0]))
    ...
    if test_ops['vsd_variation'] and cfg['data'].get('chd_info') and cfg['data']['chd_info'].get('types'):
    ...
    if test_ops['shape_gen']:
        ...
        print("DEBUG: Entering test_ops['shape_gen'] block") # DEBUG PRINT
        print("DEBUG: cfg['data']['chd_info'] before get_type_dict in shape_gen:", cfg['data'].get('chd_info')) # DEBUG PRINT
        stats = get_shape_distribution(cfg)
        
        if cfg['data'].get('chd_info'):  #type?dict нужен если нет то подставные значения
            type_dict = get_type_dict(cfg['data']['chd_info'], get_new_type=False)
            print(f"DEBUG: type_dict created from cfg['data']['chd_info']: {type_dict}") # DEBUG PRINT
        else: 
            type_dict = {'Dummy_Type_Generated': np.array([0.], dtype=np.float32)}
            print(f"DEBUG: type_dict created as Dummy_Type_Generated: {type_dict}") # DEBUG PRINT

        with torch.no_grad():
            for i, (key, chd_type) in enumerate(type_dict.items()):
                print(key)
                print(f"DEBUG: Iteration {i} in shape_gen loop. Key: {key}") # DEBUG PRINT
                print(f"DEBUG: chd_type shape: {chd_type.shape}, chd_type dtype: {chd_type.dtype}, chd_type values: {chd_type}") # DEBUG PRINT
                sample_shape_space(net, key, chd_type, cfg, lat_vecs=lat_vecs, stats=stats, num_copies=30, num_block=num_block, thresh=THRESH, get_img=False, get_noCorr=False, tester=tester)
    ...
    if test_ops['rand_type_gen'] and cfg['data'].get('chd_info'):
