...
def loss_func(sampled_gt_sdv, chd_type, type_z, type_s, outputs, kbar, i):
    recons_loss = torch.mean(((outputs.squeeze(1) - sampled_gt_sdv)**2))
    gaussian_t_loss = torch.mean(type_z**2)
    gaussian_s_loss = torch.mean(type_s**2)

    total_loss = 4 * (recons_loss) + \
           0.0*gaussian_t_loss + 0.01*gaussian_s_loss
...
def update_prediction(dataloader, lat_vecs, net, cfg, epoch):
    tester = cDeepSDFTester(device, cell_grid_size=1, out_dim=cfg['net']['out_dim'])
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            if data['filename'][0] in ['ct_1001_image', 'ct_1007_image', 'ct_1010_image', 'ct_1015_image', 'ct_1042_image', 'ct_1052_image']:
                ...
                sdf = tester.z2voxel(z_s, z_t, net.module)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, cfg['train']['epoch']):
        ...
        for i, data in enumerate(dataloader_train):
            ...
            loss, recons_noDs_loss = loss_func(point_values, chd_type, z_t, z_s, outputs, kbar, i)
