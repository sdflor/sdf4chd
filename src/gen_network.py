...
class TYPEDecoder(torch.nn.Module):
    ...
    def forward(self, z, points):
        # Handle case when z (type encoding) is None
        if z is None:
            # Return a zero tensor with the correct shape when no type information is used
            batch_size = points.shape[0]
            return torch.zeros(batch_size, points.shape[1], 1).to(points.device)
...
class DisentangledGridDecoder3D(nn.Module):
    def __init__(self, num_types=1, z_t_dim=64, \
        ...
        self.z_t_dim = z_t_dim if use_type else 0  # Store z_t_dim and set to 0 if not using types
        
        if use_type:
            self.decoder = TYPEDecoder(num_types=num_types, z_dim=self.z_t_dim, df_dim=df_dim, mlp_num=type_mlp_num, out_dim=out_dim, point_dim=39, bias=type_bias, lip_reg=lip_reg, use_diag=use_diag)
        else:
            self.decoder = TYPEDecoder(num_types=num_types, z_dim=self.z_t_dim, df_dim=df_dim, mlp_num=type_mlp_num, out_dim=out_dim, point_dim=39, bias=type_bias, lip_reg=lip_reg, use_diag=use_diag)
            
        self.flow = SHAPEDecoder(z_s_dim, self.z_t_dim, df_dim, dx_mlp_num, type_decoder=self.decoder, ins_norm=ins_norm, out_dim=out_dim, step_size=step_size, div_loss=div_loss)
        self.correction = CorrectionDecoder(z_s_dim, self.z_t_dim, df_dim, ds_mlp_num, ins_norm=ins_norm, out_dim=out_dim) 
        self.act = act_func
    ...
    def forward(self, z_s, z_s_ds, z_t, points, get_tmplt_coords=False, add_correction=True, inverse=True):
        ...
        # Handle case when not using type information
        if not self.use_type or z_t is None:
            # Skip the flow step and use points directly
            points_t = points
            div_integral = torch.tensor(0.0).to(points.device)
            grad_mag = torch.tensor(0.0).to(points.device)
        else:
            # Normal flow processing with type information
        ...
        # When not using types, pass zero tensor instead of z_t
        if not self.use_type or z_t is None:
            z_t = torch.zeros(points.shape[0], self.z_t_dim).to(points.device) if self.z_t_dim > 0 else None
...
class SDF4CHD(nn.Module):
    def __init__(self, in_dim=0, \
        ...
        if use_type:
            ...
            z_t_dim = 0  # No type dimension if not using types
            
        self.decoder = DisentangledGridDecoder3D(num_types=num_types, z_t_dim=z_t_dim, z_s_dim=z_s_dim, df_dim=latent_dim, 
                                                type_mlp_num=type_mlp_num, ds_mlp_num=ds_mlp_num, dx_mlp_num=dx_mlp_num, 
                                                out_dim=out_dim, ins_norm=ins_norm, lip_reg=lip_reg, step_size=step_size, 
                                                use_diag=use_diag, use_type=use_type, div_loss=div_loss, act_func=act_func)
        self.in_dim = in_dim
        self.use_diag = use_diag
        self.act = act_func
    
    def forward(self, z_s, z_s_ds, points, chd_type=None, get_tmplt_coords=False):
        if self.use_type:
        ...
        if self.training:
            ...
            return outputs, z_t if self.use_type else outputs
        else:
            if get_tmplt_coords:
