...
class DeepSDF(nn.Module):
    ...
    # input: N x (L+3)
    def forward(self, chd_type, xyz, grid):
        if self.ins_norm:
            grid = self.norm(grid)
        ...
        type_xyz = F.grid_sample(chd_type, xyz_s, padding_mode='border', align_corners=True)  # (B, 32, 1, 1, 4096)
        type_xyz = type_xyz.squeeze(2).squeeze(2).permute(0, 2, 1)
        input = torch.cat([type_xyz, shape_xyz, xyz_p], dim=-1)
...
class cDeepSDF(nn.Module):
    def __init__(self, \
            ...
            type_bias=True, \
            lip_reg=True):
        super(cDeepSDF, self).__init__()
        self.type_encoder = TypeEncoder(in_dim=num_types, df_dim=latent_dim, mlp_num=type_mlp_num, out_dim=z_t_dim)
        self.decoder = DeepSDF(out_dim=out_dim, latent_size=z_t_dim+z_s_dim, z_s_dim=z_s_dim, ins_norm=ins_norm)
    
    def forward(self,z_s, points, chd_type):
        z_t = self.type_encoder(chd_type)
        outputs = self.decoder(z_t, points, z_s)
...
class cDeepSDFTester(Tester):
    
    def z2voxel(self, z_s, z_t, network):
        ...
        for i in range(frame_batch_num):
            ...
            with torch.no_grad():
                model_out_ = network.decoder(z_t, point_coord.flip(-1), z_s)
        ...
        while len(queue) > 0:
            ...
            with torch.no_grad():
                model_out_batch_ = network.decoder(z_t, cell_coords.flip(-1), z_s)
